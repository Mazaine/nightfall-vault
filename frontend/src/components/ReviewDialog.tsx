...
export function ReviewDialog({ auctionTitle, onSubmit }: ReviewDialogProps) {
  ...
  return (
    <div className="dialog-backdrop" role="presentation">
      <form ref={dialogRef} className="side-panel report-dialog review-dialog" role="dialog" aria-modal="true" aria-labelledby={titleId} onKeyDown={keyDown} onSubmit={submit}>
        ...
        <p className="form-message">Az értékelés kötelező. Az alapértelmezett érték 5 csillag.</p>
        <label>Értékelés<select ref={ratingRef} value={rating} required onChange={(event) => setRating(Number(event.target.value))}>{[5, 4, 3, 2, 1].map((value) => <option value={value} key={value}>{value} csillag</option>)}</select></label>
