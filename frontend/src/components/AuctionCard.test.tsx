...
const mocks = vi.hoisted(() => ({ placeAuctionBid: vi.fn(), withdrawAuctionBid: vi.fn(), showToast: vi.fn() }));
vi.mock("../AuthContext", () => ({ useAuth: () => state }));
vi.mock("../NotificationContext", () => ({ useNotifications: () => ({ showToast: mocks.showToast }) }));
...
describe("AuctionCard", () => {
  ...
  it("a Licitálok a következő licitlépcsőt küldi navigáció nélkül", async () => {
    ...
    expect(mocks.showToast).toHaveBeenCalledWith({ title: "Sikeres licit", message: "Az új aktuális licit 1300 Ft.", targetUrl: "/auctions/7" });
  ...
  it("a Lecsapom a villámárat küldi és lezárja a kártyát", async () => {
    ...
    expect(mocks.showToast).toHaveBeenCalledWith({ title: "Sikeres villámvásárlás", message: "Megnyerted az aukciót villámáron.", targetUrl: "/auctions/7" });
    expect(screen.queryByRole("button", { name: "Licitálok" })).not.toBeInTheDocument();
  });

  it("az eladó értékelését csillagokkal, a licitszámot külön jeleníti meg", () => {
    render(<MemoryRouter><AuctionCard item={item} index={0} detailPath="/auctions/7" /></MemoryRouter>);
    expect(screen.getByText((_, element) => element?.classList.contains("seller-identity") === true && element.textContent === "Eladó: Anna Kártyabarlang")).toBeInTheDocument();
    expect(screen.getByLabelText("4 csillag az 5-ből, 0 értékelés")).toHaveTextContent("★★★★☆");
    expect(screen.getByLabelText("2 licit érkezett")).toHaveTextContent("2 licit");
  });
